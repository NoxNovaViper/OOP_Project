...
void FlyngFoogafoog::update(float Time) {
    if (Rolling) {
        Botom::update(Time);
        return;
    }
    ...
    if (!isFlying) {
        ...
        x += speed_x * snow_speed_multiplier() * Time;
        y += speed_y * snow_speed_multiplier() * Time;
        //Bounce off
        if ((x < 50) || (x > 750)) {
            speed_x = -speed_x;
        }
...
void FlyngFoogafoog::draw(sf::RenderWindow& window) {
    ...
    const Texture* tex = variant ? &Assets::FFF_red_idle : &Assets::FFF_fly_t[0];
    if (frame < 3) { 
        if (variant) tex = &Assets::FFF_red_fly[frame % 2];
        else tex = &Assets::FFF_fly_t[frame % 3];
    }
