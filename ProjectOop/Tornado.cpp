...
void Tornado::draw(sf::RenderWindow& window) {
    ...
    const Texture* tex = variant ? &Assets::tornado_red_idle : &Assets::tornado_idle_t; 
    if (frame < 3) {
        if (variant) tex = &Assets::tornado_red_run[frame % 3];
        else tex = &Assets::tornado_run_t[frame % 3]; 
    }
